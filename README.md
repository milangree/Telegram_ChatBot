...
基于 **Cloudflare Pages + Pages Functions + Vue 3 (Vuetify 3)** 的 Telegram 双向聊天机器人。

支持 **Cloudflare Pages** 和 **Docker** 两种部署方式，存储层支持 **KV / D1 / Hyperdrive（PostgreSQL & MySQL）** 自由切换。

用户私聊 Bot → 自动在话题群组创建话题 → 管理员回复 → 实时转发回用户。
